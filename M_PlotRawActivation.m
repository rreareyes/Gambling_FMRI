...
%% Ask if you want to generate plots
[options.Plots, ~] = listdlg('ListString', {'Yes', 'No'}, 'Name', 'Do you want to plot the data?');
...
names.TableRoi = {'Precun', 'RightAI', 'RightIFG', 'RightPPC', 'RightDLPFC', 'RightVMPFC', 'DMPFC'};
names.TableDes = {'Pmax_Adlt', 'NonPmax_Adlt', 'Struc_Adlt', 'Rand_Adlt', 'Zero_Adlt', 'Neg_Adlt', ...
                  'Pmax_ADO', 'NonPmax_ ADO', 'Struc_ADO', 'Rand_ADO', 'Zero_ADO', 'Neg_ADO'};
...
for iDesign = 1:size(folder.ConList, 2)
    data.DesignPaths{iDesign, 1} = fullfile(folder.ConList(iDesign).folder, folder.ConList(iDesign).name, 'SPM.mat');
end 

%% Extract activation values from each contrast and ROI
for iDesign = 1:size(data.DesignPaths, 1)
    data.Rois            = maroi(roi.CleanPath);
    data.Design          = mardo(data.DesignPaths{iDesign});  % make mardo design object
    
    data.Activation(iDesign, :)       = get_marsy(data.Rois{:}, data.Design, 'mean'); % extract data into marsy data object
    data.MeanActivation{iDesign, :}   = summary_data(data.Activation(iDesign));
    data.RegionActivation(iDesign, :) = region_data(data.Activation(iDesign));
end

summary.RegionActivation = cell2table(data.RegionActivation, 'VariableNames', names.TableRoi, 'RowNames', names.TableDes);

for iDesign = 1:size(folder.ConList, 2)
    for iROI = 1:size(roi.List, 1)
        summary.MeanActivation{iDesign, iROI} = data.MeanActivation{iDesign, 1}(:, iROI);
    end
end
summary.MeanActivation = cell2table(summary.MeanActivation, 'VariableNames', names.TableRoi, 'RowNames', names.TableDes);
...
if options.Plots == 1
    ...
    end %end loop of plots
    
end %end if statement for plotting the data
